...
@section('breadcrumb')
    <div class="page-header">
        <div class="page-title">
            <h3>{{ __('group.Group Types Table') }}</h3>
        </div>
        <div class="dropdown filter custom-dropdown-icon">
            ...
            <div class="dropdown-menu dropdown-menu-right" aria-labelledby="filterDropdown">
                ...
                <a class="dropdown-item" data-value="<span>Show</span> : Daily Analytics"
                   href="{{ route('admin.group_types.index') }}">{{ __('group.Group Types') }}</a>
                {{-- <a class="dropdown-item" data-value="<span>Show</span> : Weekly Analytics"
                    href="{{ route('admin.group_types.create') }}">{{ __('group.Create Group Type') }}</a> --}}
            </div>
        </div>
    </div>
@endsection
...
@section('content')
                    ...
                    <div class="widget-header">
                        <div class="row align-items-center">
                            <div class="col-xl-10 col-md-10 col-sm-10 col-10">
                                <h4>{{ __('group.Group Types') }}</h4>
                            </div>
                            <div class="col-xl-2 col-md-2 col-sm-2 col-2">
                                <a class="btn btn-primary float-right" data-toggle="modal"
                                   data-target="#creatGroupTypeModal">{{ __('group.Create Group Type') }}</a>
                            </div>
                        </div>
                    </div>
    ...
    @include('pages.groupType.createModal')

    @include('pages.groupType.editModal')
@endsection
